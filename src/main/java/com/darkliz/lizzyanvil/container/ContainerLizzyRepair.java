...
public class ContainerLizzyRepair extends Container {
	...
	public ContainerLizzyRepair(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn,
			EntityPlayer player) {
		...
		this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47) {
			 ...
			 */
			// removed -- && ContainerLizzyRepair.this.maximumCost > 0 -- to
			// allow the cost to be 0
			public boolean canTakeStack(EntityPlayer playerIn) {
				return (playerIn.capabilities.isCreativeMode
						|| playerIn.experienceLevel >= ContainerLizzyRepair.this.maximumCost) && this.getHasStack()
						|| ContainerLizzyRepair.this.renamingOnly;
			}

			// Causes a bug with Simple XP Storage when mana crystals are used
			// after using the anvil. this seems to be happening because the
			// above function doesn't actually set the xp total to the proper
			// amount. It only sets the level. Solution: recalculate xp total in
			// mana crystal code.
			...
			public void onPickupFromSlot(EntityPlayer playerIn, ItemStack stack) {
				...
				ContainerLizzyRepair.this.inputSlots.setInventorySlotContents(0, (ItemStack) ItemStack.EMPTY);
				...
				if (!playerIn.capabilities.isCreativeMode && !worldIn.isRemote
						&& iblockstate.getBlock() == LizzyAnvilBlocks.lizzy_anvil
					...
					if (k > 2) {
						...
						worldIn.playEvent(1020, blockPosIn, 0);
					} else {
						worldIn.setBlockState(blockPosIn,
								iblockstate.withProperty(BlockLizzyAnvil.DAMAGE, Integer.valueOf(k)), 2);
						worldIn.playEvent(1021, blockPosIn, 0);
					}
	...
	public void onCraftMatrixChanged(IInventory inventoryIn) {
		...
		if (inventoryIn == this.inputSlots) {
			...
			if (this.heatRequired && !this.world.isRemote) {
				...
				// check for heat source when something is put into the right
				// slot if a heat check has not already been done (this clears
	...
	public void updateRepairOutput() {
		...
		if (itemstack.isEmpty()) {
			...
			} else // itemstack2 != null
			{
				...
				if (!this.hasHeatSource && this.heatRequired && itemstack1.isItemStackDamageable()) // if
																									// heat
																									// is
																									// required
																									// and
				...
				isBOOKInRight = itemstack2.getItem() == Items.ENCHANTED_BOOK
						&& Items.ENCHANTED_BOOK.getEnchantments(itemstack2).tagCount() > 0;
				isBOOKInLeft = itemstack1.getItem() == Items.ENCHANTED_BOOK
						&& Items.ENCHANTED_BOOK.getEnchantments(itemstack1).tagCount() > 0;

				boolean isUnenchanting = false;

				int repairBonusPercent = Config.repairBonusPercent;
				Item unenchantItem = Item.getItemById(Config.unenchantItemID) != null
						? Item.getItemById(Config.unenchantItemID) : Items.REDSTONE; // Items.redstone
																						// ID
																						// is
																						// 331
				...
				if (itemstack1.isItemStackDamageable() && (itemstack1.getItem().getIsRepairable(itemstack, itemstack2)
						|| AnvilMethods.getIsRepairableAdditions(itemstack, itemstack2))) {
					int repairBonus;
					if (repairBonusPercent <= 0) {
						repairBonus = 0;
					} else {
						int percentPerMatierial = Math
								.max(repairBonusPercent / AnvilMethods.getMaterialCostFactor(itemstack1), 1); // a
																												// %
																												// per
																												// material
																												// used,
																												...
																												// minimum
																												// 1
						repairBonus = Math.max(itemstack1.getMaxDamage() * percentPerMatierial / 100, 1); // a
																											// %
																											// of
																											// the
																											// max
					...
					j = Math.min(itemstack1.getItemDamage(),
							itemstack1.getMaxDamage() / AnvilMethods.getMaterialCostFactor(itemstack1) + repairBonus); // j
																														// =
																														// the
																														// lower
																														// of
																				...
																				// repair
					{
						...
						j = Math.min(itemstack1.getItemDamage(),
								itemstack1.getMaxDamage() / AnvilMethods.getMaterialCostFactor(itemstack1)
										+ repairBonus);
					}
					this.materialCost = k;
					...
					// if not an enchanted BOOK in right slot and (not the same
					// type of item in both slots or not a damageable item in
					// left slot) and not an UNENCHANT_ITEM or a normal BOOK in
					// the right slot)
					if (!isBOOKInRight
					...
					// if its not an enchanted BOOK in right slot and it is a
					// damageable item or enchanted BOOK in left slot
					if (!isBOOKInRight) // &&
										...
										// || isBOOKInLeft))
					{
						// UNENCHANTING
						// ------------------------------------------------------------------------------------------------------------------
						if (itemstack2.getItem() == unenchantItem) // &&
																	// (itemstack1.isItemEnchanted()
																	// ||
																	// isBOOKInLeft))
						{
							isUnenchanting = true;

							if (!itemstack2.hasDisplayName()) // if the
																// unenchant
																// item doesn't
																// have a custom
																// name (its an
																// unnamed one)
							{
								...
								if (itemstack1.getItem() == Items.ENCHANTED_BOOK)
									itemstack1 = new ItemStack(Items.BOOK); // set
																			// output
																			// to
																			// a
																			// regular
																			// BOOK
							} else // the item has a custom name, check to see
									// if it matches the name of one of the
									// enchantments; if so, remove that
									// enchantment, leaving the rest
							{
								String slot2DisplayName = itemstack2.getDisplayName();
								int enchToRemoveID = AnvilMethods.getMatchingEnchantmentIDByName(map1,
										slot2DisplayName);

								if (map1.containsKey(Integer.valueOf(enchToRemoveID))) // check
																						// to
																						// make
																						// sure
																						// the
																						...
																						// map1
								{
									map1.remove(Integer.valueOf(enchToRemoveID)); // remove
																					// the
																					// matching
																					// enchantment
																					// from
									...
									if (itemstack1.getItem() == Items.ENCHANTED_BOOK
											&& Items.ENCHANTED_BOOK.getEnchantments(itemstack1).tagCount() <= 1) // if
																													// only
																													// one
																													// enchantment
																													// exists
									{
										itemstack1 = new ItemStack(Items.BOOK); // set
											...
											&& Items.ENCHANTED_BOOK.getEnchantments(itemstack1).tagCount() > 1) {
						...
						// TRANSFERRING
						// ------------------------------------------------------------------------------------------------------------------
						else if (itemstack2.getItem() == Items.BOOK && itemstack1.isItemStackDamageable()
								&& itemstack1.isItemEnchanted()) {
							itemstack1 = new ItemStack(Items.ENCHANTED_BOOK); // set
																				// output
																				// to
																				// an
																				// enchanted
							...
							// if the normal BOOK has a custom name, check to
							// see if it matches the name of one of the
							// enchantments; if so, transfer only that
							// enchantment
							if (itemstack2.hasDisplayName()) {
								String slot2DisplayName = itemstack2.getDisplayName();
								int enchantmentToTransferID = AnvilMethods.getMatchingEnchantmentIDByName(map1,
										slot2DisplayName);
							...
							// otherwise the normal BOOK has no display name so
							// transfer all enchantments (cost handled properly
							// in the enchantments section below)
						...
						// ------------------------------------------------------------------------------------------------------------------
						else if (itemstack1.isItemStackDamageable() && !isBOOKInRight
								&& itemstack2.getItem() != unenchantItem && itemstack2.getItem() != Items.BOOK) {
							int itemDamage;
							j = itemstack.getMaxDamage() - itemstack.getItemDamage(); // the
																						// amount
																						// of
																						// uses
																						// the
																						...
																						// has
																						// left
							k = itemstack2.getMaxDamage() - itemstack2.getItemDamage(); // the
																						// amount
																						// of
																						// uses
																						// the
																						...
																						// has
																						// left
							l = k + itemstack1.getMaxDamage() * repairBonusPercent / 100; // k
																							// +
																							// a
																							// %
																							// of
							...
							if (itemDamage < itemstack1.getMetadata()) {
								...
								// If neither item is enchanted, cost is 0 (if
								// one or both are enchanted, the cost is
								// handled by the enchantment stuff)
					...
					// Go through all the enchantments on the item in the right
					// slot
					while (iterator1.hasNext()) {
						...
						if (enchantment2 != null) {
							// get the levels of the enchantments and compare
							// them
							...
							} else // they are not the same, take the highest
									// one
							{
							...
							if (itemstack.getItem() == Items.ENCHANTED_BOOK) // was:
																				// if
							 ...
							 * 
							 * 
							 * NEEDS CHECKING FOR FUNCTIONALITY
							 */
							// go through the enchantments on the item to be put
							// in the output slot (the item in the left slot)
							...
							while (iterator.hasNext()) {
								...
								// if not the same enchantment ID and the two
								// cannot apply together
								if (enchantment1ID != enchantment2ID && !(enchantment2.func_191560_c(enchantment1)
										&& enchantment1.func_191560_c(enchantment2))) // Forge
																						// BugFix:
																						// Let
																						// Both
																						// enchantments
							...
							if (flag8) {
								...
								map1.put(Integer.valueOf(enchantment2ID), Integer.valueOf(enchantment2Level)); // put
																												// the
																												// new
																												// enchantment
																												// and
				...
				if (isUnenchanting) {
					baseCost = 1;
				} else if (itemstack1.isItemStackDamageable() && isBOOKInRight) // use
																				// the
																				// value
																				// of
																				// the
				...
				} else if (isBOOKInLeft && isBOOKInRight) // use the lower of
															// the two BOOK
															// values
				{
					Map map0 = EnchantmentHelper.getEnchantments(itemstack1); // this
																				// is
																				// the
																				// enchantment
																				// map
					...
					baseCost += baseValue; // use the value of the enchantments
											// on the output item
				}
			...
			if (!itemstack2.isEmpty()) {
				...
				// point)
				if (isBOOKInRight && !AnvilMethods.isBookCompatibleWithItem(itemstack1, itemstack2) && !isBOOKInLeft) // Original
																														// method
																														// didn't
																														// seem
																														// to
