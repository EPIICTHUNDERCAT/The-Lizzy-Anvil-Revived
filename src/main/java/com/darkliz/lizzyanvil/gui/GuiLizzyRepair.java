...
public class GuiLizzyRepair extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (this.anvil.maximumCost > 0) {
			...
			if (!this.anvil.hasHeatSource && this.anvil.heatRequired && !this.anvil.getSlot(2).getHasStack()) {
				...
				k = 16736352;
			} else if (costLimit > 0 && this.anvil.maximumCost >= costLimit
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		this.drawTexturedModalRect(k + 59, l + 20, 0, this.ySize + (this.anvil.getSlot(0).getHasStack() ? 0 : 16), 110,
				16);
	 ...
	 *            The container whose contents are to be sent to the player.
	 * @param itemsList
	 ...
	 * 
	 * @param containerToSend
	 *            The container that is to be updated on the client.
	 * @param slotInd
	 *            The slot index that is to be updated.
	 * @param stack
	 ...
	 * 
	 * @param containerIn
	 *            The container sending a progress bar update.
	 * @param varToUpdate
	 *            The integer corresponding the variable to be updated.
	 * @param newValue
